...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            // 'role' => 'required|string|in:super_admin,admin,modérateur', // Vérifie que le rôle est valide
        ...
        if (Auth::user()->role == 'super_admin') {
            $rules["role"] = "required|string|in:super_admin,admin,modérateur";
        }
    ...
    public function updateMe(Request $request)
    {
        ...
        if ($request->password && $authUser->role != "super_admin") {
        ...
        try {
            if ($authUser->role != "super_admin" && $request->password && $request->old_password) {
    ...
    {
        $user = User::loadForDiocese()->with('diocese')->orderBy('created_at', 'desc')->get();
        return response()->json([
            'user' => $user,
        ]);
    }
